...
export default function CpfDrawer({ row, onSave, onClose }) {
  ...
  async function handleSave() {
    ...
    setError("");
    try {
      await onSave({ cpfNumber, boxType, offer, reservationList: reservations });
    } catch (requestError) {
      setError(requestError.response?.data?.message || "Impossible d’enregistrer le formulaire CPF.");
    } finally {
      setSaving(false);
    }
  }
    ...
    <>
      ...
      <div className="cpf-drawer">
        ...
        <div className="cpf-drawer-body">
          ...
          {error && <p className="cpf-drawer-error" role="alert">{error}</p>}
        ...
        <div className="cpf-drawer-footer">
          ...
          <button className="cpf-footer-btn cpf-footer-btn--save" onClick={handleSave} disabled={saving}>
