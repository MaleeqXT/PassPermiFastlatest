...
import { addCandidate } from "../../redux/reducers/candidateSlice.jsx";
...
export default function NewCandidate({ onBack }) {
  const { addCandidate1 } = useCandidates();
  ...
  const [form, setForm] = useState({
    ...
    neph: "", date_code: "", password: "", password_confirmation: "",
    status: "", is_cpf: 1, boite_type: 0,
    balance: "0", estimation: "0",
  });
  ...
  function validate() {
    ...
    if (form.neph && !/^\d{12}$/.test(form.neph)) nextErrors.neph = true;
        ...
        <div className="nc-card">
          ...
          <div className="nc-grid-2">
            <div className={`nc-field ${errors.neph ? "error" : ""}`}>
              ...
              <input className="nc-input" value={form.neph} onChange={e => set("neph", e.target.value.replace(/\D/g, "").slice(0, 12))} inputMode="numeric" maxLength={12} />
            </div>
            <div className="nc-field">
              <label>Date du code</label>
              <input className="nc-input" value={form.date_code} onChange={e => set("date_code", e.target.value)} type="date" placeholder="jj/mm/aaaa" />
            </div>
          </div>
      ...
      <aside className="nc-right">
        ...
        <div className="nc-right-card nc-right-row">
          ...
          <label className="nc-toggle">
            <input type="checkbox" checked={form.is_cpf} onChange={e => set("is_cpf", e.target.checked ? 1 : 0)} />
        ...
        <div className="nc-right-card nc-right-row">
          ...
          <div className="nc-box-toggle">
            {["Manuel", "Auto"].map(opt => (
              <button
                key={opt}
                className={`nc-box-opt ${form.boite_type === (opt === "Auto" ? 1 : 0) ? "sel" : ""}`}
                onClick={() => set("boite_type", opt === "Auto" ? 1 : 0)}
              >
                {opt}
              </button>
            ))}
