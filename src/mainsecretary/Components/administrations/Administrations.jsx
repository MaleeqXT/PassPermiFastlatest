...
// ── Default sample data ───────────────────────────────────────────────────────
const initialAdmins = [
  { id:1, nom:"Admin",  prenom:"Super", phone:"-",             date:"2025-05-12", status:"active"   },
  { id:2, nom:"Dupont", prenom:"Jean",  phone:"+33 6 12 34 56", date:"2025-06-01", status:"active"   },
  { id:3, nom:"Martin", prenom:"Claire",phone:"+33 7 98 76 54", date:"2025-07-15", status:"archived" },
];
...
export default function Administrators({ admins: propAdmins, setAdmins: propSetAdmins, onSelectAdmin }) {
  ...
  const admins    = propAdmins    ?? localAdmins;
  const setAdmins = propSetAdmins ?? setLocalAdmins;
  ...
  const counts = {
    ...
    archive: admins.filter(s => s.status === "archived").length,
  };
                ...
                <tr key={s.id} className={s.status === "archived" ? "cand-row-archived" : ""}>
                  ...
                  <td>
                    <div className="cand-candidate-cell">
                      {s.photo
                        ? <img src={s.photo} alt={s.prenom} className="cand-avatar" style={{ objectFit:"cover", borderRadius:"50%", width:34, height:34 }} />
