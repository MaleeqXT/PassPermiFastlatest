    ...
    <div className="cand-row-menu-wrapper" ref={ref}>
      ...
      {open && (
        <div className="cand-row-menu-dropdown">
          <button className="cand-row-menu-item" onClick={() => { setOpen(false); navigate("/invoice"); }}>
...
export default function Billing() {
  ...
  let data = INVOICES.filter((invoice) => !search || invoice.name.toLowerCase().includes(search.toLowerCase()));
  if (sort) {
    data = [...data].sort((a, b) => {
                      ...
                      <td>
                        <button
                          ...
                          onClick={() => setSelectedMonitor({
                            id: invoice.id,
                            prenom: invoice.name.split(" ")[0] || "",
                            nom: invoice.name.split(" ").slice(1).join(" ") || invoice.name,
                            status: invoice.status === "paid" ? "active" : "onhold",
                          })}
                        >
                          Voir le dossier
                        </button>
