...
function ProposalCard({ proposal, onSelect }) {
  ...
  const ini = initials(proposal.candidateName);
  const pedagogicalStatus = proposal.pedagogicalReview?.status
    ? (proposal.pedagogicalReview.status === "absent" ? "Absent" : "Présent")
    : null;

  return (
    <button type="button" className="psl-proposal-card" onClick={() => onSelect?.(proposal)}>
      ...
      <div className="psl-card-body">
        ...
        <div className="psl-card-text">
          ...
          <div className="psl-card-main">
            ...
            {" "}à <strong>{proposal.location}</strong>
          </div>
          <div className="psl-card-meta">{proposal.time} - {proposal.offer}</div>
          {proposal.pedagogicalReview?.note && (
            <div className="psl-card-meta">
              {pedagogicalStatus ? `${pedagogicalStatus} - ` : ""}{proposal.pedagogicalReview.note}
            </div>
          )}
        </div>
      </div>
...
export default function ProposedSessionsList({ onBack, proposals = PROPOSALS, onSelectProposal }) {
  const [query, setQuery] = useState("");

  const filtered = proposals.filter((proposal) => (
    proposal.candidateName.toLowerCase().includes(query.toLowerCase())
    || proposal.location.toLowerCase().includes(query.toLowerCase())
    || proposal.offer.toLowerCase().includes(query.toLowerCase())
  ));
  ...
  return (
    <div className="psl-page">
      ...
      <p className="psl-count">{filtered.length} proposition{filtered.length !== 1 ? "s" : ""}</p>

      <div className="psl-timeline">
        {grouped.length === 0 && (
          <div className="psl-empty">Aucune proposition ne correspond à « {query} »</div>
        )}
            ...
            {groupedProposals.map((proposal, index) => (
              <div key={proposal.id} className="psl-entry">
                ...
                <div className="psl-entry-card">
                  <ProposalCard proposal={proposal} onSelect={onSelectProposal} />
                </div>
              </div>
            ))}
          </div>
