...
const INITIAL_SKILLS = [
  { id:1, position:1, name:"Maîtrise de la conduite du véhicule en trafic faible ou nul",                             label:"Compétence 1", status:"active"   },
  { id:2, position:2, name:"Compréhension de la route et conduite dans des conditions normales",                       label:"Compétence 2", status:"active"   },
  { id:3, position:3, name:"Conduite dans des conditions difficiles et partage de la route avec les autres usagers",   label:"Compétence 3", status:"active"   },
  { id:4, position:4, name:"Pratiquer une conduite autonome, sûre et économique",                                      label:"Compétence 4", status:"active"   },
];
  ...
  return (
    <div className="skills-card">
      ...
      <div className="skills-card-actions">
        <button className="skills-file-btn" onClick={onDetail}>Voir la dossier</button>
      </div>
    </div>
  );
}
...
export default function CandidateSkills() {
  ...
  if (detailSkill) return <SkillDetail skillName={detailSkill.name} onBack={() => setDetailSkill(null)} />;
  ...
  function handleSave(data) {
    if (drawerMode === "add")
      setSkills(prev => [...prev, { ...data, id: Date.now() }].sort((a, b) => a.position - b.position));
    else
      setSkills(prev => prev.map(s => s.id === data.id ? data : s).sort((a, b) => a.position - b.position));
    setDrawerMode(null); setEditSkill(null); showToast();
  }

  function handleDeleteConfirm() {
    setSkills(prev => prev.filter(s => s.id !== deleteTarget));
      ...
      <div className="skills-table-card">
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="skills-table">
            ...
            <tbody>
              {filtered.length === 0
                ? <tr><td colSpan={6} className="skills-empty">Aucune compétence trouvée</td></tr>
                : filtered.map((s, i) => (
                  <tr key={s.id}>
                    <td className="skd-num-col skd-num-cell">{i + 1}</td>
                    <td>
                      <button className="skills-file-btn" onClick={() => setDetailSkill(s)}>
                        Voir la fiche
                      </button>
                    </td>
                    <td><span className="skills-name-link">{s.name}</span></td>
