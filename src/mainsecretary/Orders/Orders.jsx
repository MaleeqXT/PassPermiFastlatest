...
const inRange     = (iso, s, e) => { if (!s && !e) return true; const d = new Date(iso); d.setHours(0,0,0,0); if (s && d < s) return false; if (e && d > e) return false; return true; };
...
// ── Stat Card ─────────────────────────────────────────────────────────────────
function StatCard({ label, amount, count, countLabel }) {
    ...
    <div className="ord-stat-card" style={{ backgroundImage:`url(${bginfo})` }}>
      ...
      <div className="ord-stat-body">
        <div className="ord-stat-top">
          ...
          <span className="ord-stat-pct">0%</span>
        </div>
        <div className="ord-stat-amount">{amount.toFixed(2).replace(".",",")} €</div>
        <div className="ord-stat-sub">0,00 € vs. les 30 derniers jours</div>
  ...
  return (
    <div className="ord-datepicker-anchor" ref={ref}>
      <button className="ord-date-btn" onClick={() => setOpen(o => !o)}>
        ...
        <IconChevronD />
        {selected && (
          <button className="ord-date-btn-clear" onClick={e => { e.stopPropagation(); setSelected(null); }}>
            <IconX />
          </button>
        )}
      </button>
      {open && (
        <div className="ord-payment-dropdown">
          {options.map(opt => (
            <button
              key={opt}
              className={`ord-payment-option ${selected === opt ? "ord-payment-option--active" : ""}`}
...
export default function Orders() {
  ...
  const cardData       = SAMPLE_ORDERS.filter(o => inRange(o.createdAt, cardS, cardE));
  const paidTotal      = cardData.filter(o=>o.status==="paid"    ).reduce((s,o)=>s+o.total,0);
  const pendingTotal   = cardData.filter(o=>o.status==="on_hold" ).reduce((s,o)=>s+o.total,0);
  const cancelledTotal = cardData.filter(o=>o.status==="canceled").reduce((s,o)=>s+o.total,0);
  const refundedTotal  = cardData.filter(o=>o.status==="refunded").reduce((s,o)=>s+o.total,0);

  let tableData = SAMPLE_ORDERS.filter(o => inRange(o.createdAt, tabS, tabE));
  if (!NO_FILTER_TABS.has(activeTab)) tableData = tableData.filter(o => o.status === activeTab);
  if (search) tableData = tableData.filter(o =>
    o.candidate.toLowerCase().includes(search.toLowerCase()) || o.ref.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const counts = {
    ...
    on_hold:  SAMPLE_ORDERS.filter(o=>o.status==="on_hold" ).length,
    paid:     SAMPLE_ORDERS.filter(o=>o.status==="paid"    ).length,
    refunded: SAMPLE_ORDERS.filter(o=>o.status==="refunded").length,
    canceled: SAMPLE_ORDERS.filter(o=>o.status==="canceled").length,
    auto:     SAMPLE_ORDERS.length,
    bm:       SAMPLE_ORDERS.length,
  };
  ...
  return (
    <div className="cand-page">
      ...
      <div className="ord-stats-row">
        <StatCard label="Revenus encaissés"   amount={paidTotal}      count={cardData.filter(o=>o.status==="paid"    ).length} countLabel="Ventes payées"      />
        <StatCard label="Revenus en attente"  amount={pendingTotal}   count={cardData.filter(o=>o.status==="on_hold" ).length} countLabel="Ventes en attente"  />
        <StatCard label="Revenus annulés"     amount={cancelledTotal} count={cardData.filter(o=>o.status==="canceled").length} countLabel="Ventes annulées"    />
        <StatCard label="Revenus remboursés"  amount={refundedTotal}  count={cardData.filter(o=>o.status==="refunded").length} countLabel="Ventes remboursées" />
      </div>
            ...
            {paginated.length === 0
              ...
              : paginated.map(o => {
                  ...
                  return (
                    <tr key={o.id}>
                      <td>
                        <Link to="/orders/info" style={{ textDecoration:"none" }}>
