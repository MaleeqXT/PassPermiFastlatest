...
import { useMonitors } from "../Components/monitors/MonitorsContext.jsx";
...
function StatusModal({ entry, onSave, onClose }) {
  const isProcessed = entry.status === "refused";
...
export default function ReservationCancellations() {
  const { candidates, setSelectedCandidateId } = useCandidates();
  ...
  // ── Filtrage ──
  let filtered = data;
  if (activeTab === "noproof")   filtered = data.filter(r => r.status === "refused");
  if (activeTab === "justified") filtered = data.filter(r => r.status === "cancelled");
  ...
  function openCandidateFile(row) {
    const candidate = candidates.find(
      e => `${e.prenom} ${e.nom}`.toLowerCase() === row.candidateName.toLowerCase()
    );
    if (candidate?.id) setSelectedCandidateId(candidate.id);
    setSelectedCandidate(candidate ?? {
      nom: row.candidateName.split(" ").slice(1).join(" ") || row.candidateName,
      prenom: row.candidateName.split(" ")[0] || "",
      permis: "Voiture", status: "active", balance: "0h", photo: null,
    });
  }

  function openMonitorFile(row) {
    const monitor = monitors.find(
      e => `${e.prenom} ${e.nom}`.toLowerCase() === row.monitorName.toLowerCase()
    );
    setSelectedMonitor(monitor ?? {
      id: row.id,
      prenom: row.monitorName.split(" ")[0] || "",
      nom: row.monitorName.split(" ").slice(1).join(" ") || row.monitorName || "Inconnu",
      would: "", status: "active", photo: null,
    });
  }

  // Pagination label
  const pageLabel = filtered.length === 0
    ? "0 élément"
    : `${(page - 1) * PAGE_SIZE + 1} – ${Math.min(page * PAGE_SIZE, filtered.length)} sur ${filtered.length} éléments`;
    ...
    <div className="cand-page rc-page">
      ...
      <div className="cand-table-card">
        ...
        <div style={{ overflowX: "auto" }}>
          <table className="cand-table" style={{ minWidth: 1200 }}>
            <thead>
              <tr>
            ...
            <tbody>
              {paginated.length === 0
                ...
                : paginated.map(row => (
                  <tr key={row.id}>
                    <td><button className="cand-file-btn" onClick={() => openCandidateFile(row)}>Voir la dossier</button></td>
                    ...
                    <td className="rc-doc">Aucun document</td>
                  </tr>
                ))
              }
            </tbody>
        ...
        <div className="cpf-pagination">
          ...
          <button className="cpf-page-btn" onClick={() => setPage(p => Math.min(totalPages || 1, p + 1))} disabled={page >= totalPages}>
            <IconChevR />
          </button>
        </div>
      </div>
