...
import { fetchBaskets } from "../redux/reducers/basketsSlice";
...
function normaliseBasket(cart) {
  const user = cart.student?.user ?? {};
  const details = cart.cart_details ?? cart.cartDetails ?? [];
  const totals = details.reduce((result, detail) => {
    const quantity = Number(detail.quantity ?? 1) || 1;
    const price = Number(detail.price ?? detail.offer?.final_price ?? detail.offer?.discounted_price ?? detail.offer?.original_price ?? 0) || 0;
    const balance = Number(detail.balance ?? detail.offer?.balance ?? 0) || 0;
    result.amount += price * quantity;
    result.balance += balance * quantity;
    return result;
  }, { amount: 0, balance: 0 });

  return {
    id: cart.id,
    studentId: cart.student?.id,
    prenom: user.first_name ?? "",
    nom: user.last_name ?? "",
    email: user.email ?? "",
    phone: user.phone ?? "",
    offers: details.length,
    balance: `${totals.balance} H`,
    amount: `${totals.amount.toFixed(2).replace(".", ",")} €`,
    date: formatDateTime(cart.created_at),
    status: Number(cart.status) === 2 ? "paid" : "onhold",
  };
}
...
// ── Main ───────────────────────────────────────────────────────────────────
export default function Baskets({ selectedSchoolId }) {
  const dispatch = useDispatch();
  const { list: cartList, counts, lastPage, total, loading, error } = useSelector((state) => state.baskets);
  ...
  useEffect(() => {
    const status = activeTab === "onhold" ? 1 : activeTab === "paid" ? 2 : undefined;
    dispatch(fetchBaskets({ page, status, search: search || undefined, zone_id: selectedSchoolId || undefined }));
  }, [dispatch, page, activeTab, search, selectedSchoolId]);
  ...
  const tabCounts = {
    all: counts.all_count ?? total ?? 0,
    onhold: counts.onhold_count ?? 0,
    paid: counts.paid_count ?? 0,
  };

  let data = cartList.map(normaliseBasket);
      ...
      <div className="bsk-tabs-search-row">
        <div className="cand-tabs-row" style={{ marginTop:0, marginBottom:0 }}>
          {TABS.map(tab => (
            ...
            >
              {tab.label}
              <span className="cand-tab-count">{tabCounts[tab.key]}</span>
            </button>
          ))}
        </div>
      ...
      <div className="cand-table-card" style={{ marginTop:24 }}>
        <div style={{ overflowX:"auto" }}>
          <table className="cand-table">
            ...
            <tbody>
              {loading
                ? <tr><td colSpan={9} className="cand-empty">Chargement des paniers…</td></tr>
                : error
                  ? <tr><td colSpan={9} className="cand-empty">Impossible de charger les paniers.</td></tr>
                : paginated.length === 0
                ? <tr><td colSpan={9} className="cand-empty">Aucune entrée trouvée</td></tr>
                : paginated.map(b => (
                  <tr key={b.id}>
                    <td>
                      <Link to={`/Basketsinfo/${b.id}`}>
