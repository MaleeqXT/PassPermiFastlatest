...
import "./ProposalDetailDrawer.css";
...
export default function ProposalDetailDrawer({ proposal, onClose }) {
  ...
  const initials = getInitials(proposal.candidate);

  const handleDelete = async () => {
    setDeleting(true);
    setDeleteError("");
    try {
      await dispatch(deleteMonitorProposal({ proposalId: proposal.id })).unwrap();
      onClose();
    } catch (error) {
      setDeleteError(error?.message || "Impossible de supprimer la proposition.");
      setDeleting(false);
    }
  };
      ...
      <div className="sdr-drawer">
        ...
        <div className="sdr-body">
          ...
          <div className="pd-avatar-card">
            {avatarUrl ? (
              <img src={avatarUrl} alt={proposal.candidate} className="pd-avatar" style={{ borderRadius: "50%", objectFit: "cover", width: "42px", height: "42px" }} />
            ) : (
              <div className="pd-avatar">{initials}</div>
            )}
            <div className="pd-avatar-info">
              <div className="pd-name">{proposal.candidate}</div>
              {proposal.email && <div className="pd-phone">{proposal.email}</div>}
            </div>
          </div>
          ...
          <div className="sdr-section-card">
            <div className="sdr-detail-rows">
              <div className="sdr-detail-row">
                ...
                <span className="sdr-detail-val">{formatDate(proposal.date)}</span>
              </div>
              <div className="sdr-detail-row">
                <span className="sdr-detail-icon"><IconClock /></span>
                <span className="sdr-detail-label">Heure</span>
                <span className="sdr-detail-colon">:</span>
                <span className="sdr-detail-val">{proposal.startTime} à {proposal.endTime}</span>
              </div>
            </div>
          </div>
          ...
          <div className="sdr-map-section">
            ...
            <button className="sdr-location-row" onClick={() => setShowMapModal(true)}>
              <span className="sdr-location-name">{proposal.mapLocation || proposal.place}</span>
              <IconChevR />
            </button>
          </div>
        </div>
        ...
        <div className="sdr-footer">
          {deleteError && <p style={{ color: "#ef4444", fontSize: 13, margin: "0 0 8px", textAlign: "center" }}>{deleteError}</p>}
          <button
            className="pd-cancel-btn"
            onClick={handleDelete}
            disabled={deleting}
          >
            {deleting ? "Suppression..." : "Annuler cette proposition"}
          </button>
        </div>
      </div>

      {showMapModal && (
        <MapModal location={proposal.mapLocation || proposal.place} mapUrl={proposal.mapUrl} onClose={() => setShowMapModal(false)} />
      )}
    </>
  );
}
