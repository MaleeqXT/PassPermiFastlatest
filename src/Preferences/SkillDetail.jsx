...
import {
  fetchSubCompetences,
  selectSubCompetenceList,
  selectSubCompetenceLoading,
  selectSubCompetenceAllCount,
  selectSubCompetenceActiveCount,
  selectSubCompetenceArchiveCount,
  addSubCompetence,
  updateSubCompetence,
  deleteSubCompetence
} from "../redux/reducers/subCompetencesSlice";
...
// ── Main Detail Page ──────────────────────────────────────────────────────
export default function SkillDetail({ skillId, skillName, onBack  }) {
  const dispatch = useDispatch();
  const items        = useSelector(selectSubCompetenceList);
  const loading      = useSelector(selectSubCompetenceLoading);
  const allCount     = useSelector(selectSubCompetenceAllCount);
  const activeCount  = useSelector(selectSubCompetenceActiveCount);
  const archiveCount = useSelector(selectSubCompetenceArchiveCount);
  ...
  //   useEffect(() => {
  //   dispatch(fetchSubCompetences(skillId));
  // }, [dispatch, skillId]);
  useEffect(() => {

    dispatch(fetchSubCompetences({ 
      groupId: skillId, 
      search, 
      status: activeTab === "all" ? "" : activeTab 
    }));

}, [dispatch, skillId, activeTab, search]);

  // counts backend se
  const counts = {
    all:      allCount,
    active:   activeCount,
    inactive: archiveCount,
  };
  ...
  const TABS = [
    ...
    { key:"inactive", label:"Archivé" },
  ];

 
  const filtered = [...items].sort((a, b) => a.position - b.position);
  ...
  // }
    ...
    const result = await dispatch(deleteSubCompetence(deleteTarget));
      if (deleteSubCompetence.fulfilled.match(result)) {
    dispatch(fetchSubCompetences({ groupId: skillId, search, status: activeTab === "all" ? "" : activeTab }));
    showToast();
  }
...
async function handleSave(data) {
  if (drawerMode === "add") {
    ...
    formData.append("main_competency_id", skillId);

    const result = await dispatch(addSubCompetence({ groupId: skillId, formData }));
    if (addSubCompetence.fulfilled.match(result)) {
      dispatch(fetchSubCompetences({ groupId: skillId, search, status: activeTab === "all" ? "" : activeTab }));
      showToast();
    }
    ...
    const result = await dispatch(updateSubCompetence({ id: data.id, formData }));
    if (updateSubCompetence.fulfilled.match(result)) {
      dispatch(fetchSubCompetences({ groupId: skillId, search, status: activeTab === "all" ? "" : activeTab }));
      showToast();
    }
  }
