...
export default function SkillDrawer({ mode, skill, nextPosition, onSave, onClose }) {
  ...
  useEffect(() => {
      console.log("skill status:", skill?.status);

    if (!isAdd && skill) {
      setEnabled(skill.status === "active");  // 👈 ye already hai
      setPosition(String(skill.position ?? ""));
      setName(skill.name ?? "");
      setLabel(skill.label ?? "");
    }
  }, [skill?.id,skill?.status]); // eslint-disable-line react-hooks/exhaustive-deps
