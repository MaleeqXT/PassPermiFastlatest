...
import {  fetchPlacesById, addPlacesByZone,
  selectPlacesList, selectPlacesLoading,
  selectPlacesAllCount, selectPlacesActiveCount, selectPlacesArchiveCount, updatePlaceById,deletePlaceById } from "../redux/reducers/locationSlice";
// ── Icons ─────────────────────────────────────────────────────────────────────
const IconExport = () => (
...
export default function ZonePlaces({ zone, onBack }) {
  ...
  const zoneId = selectedSchool?.id;
  const zoneName = selectedSchool?.name;



const places       = useSelector(selectPlacesList);
const loading      = useSelector(selectPlacesLoading);
const allCount     = useSelector(selectPlacesAllCount);
const activeCount  = useSelector(selectPlacesActiveCount);
const archiveCount = useSelector(selectPlacesArchiveCount);


// const selectedSchool = useSelector((state) => state.schools.selected);
console.log("selectedSchool:", selectedSchool);
console.log("zoneId:", selectedSchool?.id);
...
useEffect(() => {
  if (zoneId) {
    dispatch(fetchPlacesById({ zoneId, page: 1, search, status: activeTab === "all" ? "all" : activeTab }));
  }
}, [dispatch, zoneId, activeTab, search]);
  ...
  if (drawerMode === "add") {
    ...
    formData.append("map_url", data.mapUrl ?? "");
    formData.append("status", data.status === "active" ? 1 : 0);

    const result = await dispatch(addPlacesByZone({ zoneId, formData }));
    if (addPlacesByZone.fulfilled.match(result)) {
      dispatch(fetchPlacesById({ zoneId, page: 1, search, status: activeTab === "all" ? "all" : activeTab }));
      setToast("Lieu ajouté avec succès.");
    }
  } else {
    // update baad mein
    const formData = new FormData();
    formData.append("name", data.name);
    formData.append("url", data.mapUrl ?? "");
    formData.append("status", data.status === "active" ? 1 : 0);

    const result = await dispatch(updatePlaceById({ placeId: data.id, formData }));
    if (updatePlaceById.fulfilled.match(result)) {
      dispatch(fetchPlacesById({ zoneId, page: 1, search, status: activeTab === "all" ? "all" : activeTab }));
      setToast("L'emplacement a été mis à jour avec succès.");
    }
  }
  setDrawerMode(null);
  setEditPlace(null);
  ...
  // ── Delete confirmed ──────────────────────────────────────────────────────
      ...
      const result = await dispatch(deletePlaceById(deleteTarget.id));
        if (deletePlaceById.fulfilled.match(result)) {
      dispatch(fetchPlacesById({ zoneId, page: 1, search, status: activeTab === "all" ? "all" : activeTab }));
       setToast("Lieu supprimé avec succès.");
    }
  ...
  //   if (activeTab === "active")   return p.status == "1";
  //   if (activeTab === "archived") return p.status == "0";
  //   return true;
  // });
    ...
    <div className="zones-page">
      ...
      <div className="zones-header">
        <div className="zones-header-left">
          <h1 className="zones-title">Zones: {zoneName}</h1>
        </div>
        <button
            ...
            {pageData.length === 0
              ...
              : pageData.map(place => (
                <tr key={place.id}>
                  ...
                  <td style={{ color:"#6b7280", fontSize:13 }}>{fmtDateTime(place.created_at)}</td>
